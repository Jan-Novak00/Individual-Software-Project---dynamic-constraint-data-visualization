...
class GameCandlestickChartEventHandler(CandlesticEventHandler,GameEventHandler):
    """Game event handler for candlestick chart prediction games.

    Combines CandlesticEventHandler with GameEventHandler to support pause/unpause
    ...
    def __init__(self, plotMetadata: CandlesticPlotMetadata, solver: CandlestickChartSolver, dataViewerClass: Type[GameDataViewer]) -> None:
        """Initializes the game candlestick chart event handler.

        Args:
            plotMetadata (CandlesticPlotMetadata): Metadata for the candlestick chart.
            solver (CandlestickChartSolver): Solver containing the candlestick data.
    ...
    @inheritdocstring(CandlesticEventHandler.initializeDataView)
    ...
    @inheritdocstring(CandlesticEventHandler.on_left_down)
    def on_left_down(self, event: Event):
        if self.paused:
            return
        super().on_left_down(event)
    
    @inheritdocstring(CandlesticEventHandler.on_mouse_move)
    def on_mouse_move(self, event):
        if self.paused:
            return
        return super().on_mouse_move(event)
    
    @inheritdocstring(CandlesticEventHandler.on_right_down)
    def on_right_down(self, event: Event):
        if self.paused:
            return
        super().on_right_down(event)
    
    @inheritdocstring(CandlesticEventHandler.on_left_up)
    def on_left_up(self, event):
        if self.paused:
            return
        return super().on_left_up(event)
    
    @inheritdocstring(CandlesticEventHandler.on_right_up)
    def on_right_up(self, event: Event) -> None:
        if self.paused:
            return
        return super().on_right_up(event)

    @inheritdocstring(CandlesticEventHandler.check_cursor)
